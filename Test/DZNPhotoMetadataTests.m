...
- (void)testParsingForService:(DZNPhotoPickerControllerService)service
{
    ...
    XCTAssertFalse((metadata.id && metadata.title && metadata.thumbURL && metadata.sourceURL && metadata.detailURL && metadata.authorName && metadata.authorUsername && metadata.authorProfileURL && metadata.serviceName), @"No attribute from a metadata object should be nil. %@", metadata.id);
}
